...
TaskHandle_t OneHzHandle = NULL;
TaskHandle_t TwoHzHandle = NULL;
StaticTask_t TwoHzTCB;

#define DRAW 0
#define DONT_DRAW 1

SemaphoreHandle_t OneHzSignal = NULL;
SemaphoreHandle_t TwoHzSignal = NULL;

void vOneHzTask(void *pvParameters)
{
	char do_i_draw = DRAW;
	...
	while (1) {
		if (OneHzSignal != NULL) {
			xSemaphoreTake(OneHzSignal, portMAX_DELAY);

			if (xTaskGetTickCount() - last_toggle_time >=
			    pdMS_TO_TICKS(500)) {
				do_i_draw ^= 1;
...
void vTwoHzTask(void *pvParameters)
{
	char do_i_draw = DRAW;
	TickType_t last_toggle_time = xTaskGetTickCount();
	while (1) {
		if (TwoHzSignal != NULL) {
			xSemaphoreTake(TwoHzSignal, portMAX_DELAY);
			...
			switch (do_i_draw) {
			case DRAW:
				gfxDrawCircle(100, 50, 20, Purple);
				break;
			case DONT_DRAW:
			default:
				break;
...
{
	xTaskCreate(vOneHzTask, "One Hz Task", MY_STACK_SIZE, NULL,
		    mainGENERIC_PRIORITY, &OneHzHandle);

	TwoHzHandle = xTaskCreateStatic(vTwoHzTask, "Two Hz Task", MY_STACK_SIZE, NULL,
			  mainGENERIC_PRIORITY, my_stack, &TwoHzTCB);

	// vTaskSuspend(OneHzHandle);
	// vTaskSuspend(TwoHzHandle);
}
	...
	while (1) {
		if (DrawSignal)
			if (xSemaphoreTake(DrawSignal, portMAX_DELAY) ==
				...
				if ((xTaskGetTickCount() - xLastResetTime) >
				    500) {
...
void vStateTwoInit(void)
{
	...
	if (xSemaphoreTake(my_ball.lock, portMAX_DELAY) == pdTRUE) {
		my_ball.ball = gfxCreateBall(SCREEN_WIDTH / 2,
					     SCREEN_HEIGHT / 2, Black, 20, 1000,
					     &vPlayBallSound, NULL, NULL);
		xSemaphoreGive(my_ball.lock);
		vResetBall();
	}
}
...
{
	vTaskResume(OneHzHandle);
	// vTaskResume(TwoHzHandle);
...
#define INCREMENT_COUNT(BUTTON)                                                \
	if (buttons.buttons[SDL_SCANCODE_##BUTTON] &&                          \
...
{
	OneHzSignal = xSemaphoreCreateBinary();
	TwoHzSignal = xSemaphoreCreateBinary();
	...
	while (1) {
		if (DrawSignal)
			if (xSemaphoreTake(DrawSignal, portMAX_DELAY) ==
				...
				xSemaphoreGive(OneHzSignal);
				xSemaphoreGive(TwoHzSignal);

				// Do my drawing here
				//////
				gfxDrawCircle(OFFSET_X(SCREEN_WIDTH / 2),
					      OFFSET_Y(SCREEN_HEIGHT / 2), 20,
					      Red);
				gfxDrawFilledBox(
					OFFSET_X(SCREEN_WIDTH / 2 - 80),
					OFFSET_Y(SCREEN_HEIGHT / 2), 40, 20,
					Blue);
				coord_t my_triangle_points[3] = {
					{ .x = OFFSET_X(SCREEN_WIDTH / 2 + 40),
					  .y = OFFSET_Y(
						  SCREEN_HEIGHT / 2 +
						  20) }, //bottom left point
					{ .x = OFFSET_X(SCREEN_WIDTH / 2 + 60),
					  .y = OFFSET_Y(SCREEN_HEIGHT /
							2) }, // top point
					{ .x = OFFSET_X(SCREEN_WIDTH / 2 + 80),
					  .y = OFFSET_Y(SCREEN_HEIGHT / 2 + 20) }
				}; // bottom right point
				gfxDrawTriangle(my_triangle_points, Green);

				gfxDrawText("Hello world", OFFSET_X(string_x),
				...
				if (string_x >
				    SCREEN_WIDTH - my_strings_length) {
					string_x = SCREEN_WIDTH -
						   my_strings_length;
				...
				if (xSemaphoreTake(buttons.lock,
					...
					xSemaphoreGive(
						buttons.lock); // giving back the lock
				}
				...
				gfxDrawText(str, OFFSET_X(SCREEN_WIDTH / 2),
					    OFFSET_Y(50), Black);
	...
	while (1) {
		...
		if (udp_soc_two)
			aIOSocketPut(UDP, NULL, UDP_TEST_PORT_2,
				     (char *)&test_struct, sizeof(test_struct));
		if (tcp_soc)
