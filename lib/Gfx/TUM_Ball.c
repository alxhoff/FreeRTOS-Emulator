...
wall_t *createWall(unsigned short x1, unsigned short y1, unsigned short w, unsigned short h, float dampening, unsigned int colour, void (*callback)(void *), void *args)
{
...
void setWallProperty(wall_t *wall, unsigned short x, unsigned short y, unsigned short width, unsigned short height, unsigned char flags)
{
    if (flags & 1) { // Set X
        wall->x1 = x;
        wall->x2 = x + wall->w;
...
ball_t *createBall(unsigned short initial_x, unsigned short initial_y, unsigned int colour, unsigned short radius, float max_speed, void (*callback)(void *), void *args)
{
...
unsigned char collideWall(ball_t *ball, wall_t *wall, unsigned char collision_type, void (*callback)(void *), void *args)
{
    if (callback) {
        callback(args);
    }
...
unsigned char handleCollision(ball_t *ball, void *object, unsigned char flag, void (*callback)(void *), void *args)
{
    unsigned char ret = 0;
    switch (flag) {
        case COLLIDE_WALL:
            // Coming from:
            // Above wall
            if (BALL_BOTTOM_POINT_Y >= WALL->y1 && BALL_TOP_POINT_Y < WALL->y1 && BALL_BOTTOM_POINT_X >= WALL->x1 && BALL_BOTTOM_POINT_X <= WALL->x2 && ball->dy > 0) {
            ...
            // Below wall
            if (BALL_TOP_POINT_Y <= WALL->y2 && BALL_BOTTOM_POINT_Y > WALL->y2 && BALL_TOP_POINT_X >= WALL->x1 && BALL_TOP_POINT_X <= WALL->x2 && ball->dy < 0) {
                ball->f_y = WALL->y2 + ball->radius;
                collideWall(ball, WALL, COLLIDE_WALL_BOTTOM, callback, args);
                ret = 1;
            }
            // Left of wall
            if (BALL_RIGHT_POINT_X >= WALL->x1 && BALL_LEFT_POINT_X < WALL->x1 && BALL_RIGHT_POINT_Y >= WALL->y1 && BALL_RIGHT_POINT_Y <= WALL->y2 && ball->dx > 0) {
                ball->f_x = WALL->x1 - ball->radius;
                collideWall(ball, WALL, COLLIDE_WALL_RIGHT, callback, args);
                ret = 1;
            }
            // Right of wall
            if (BALL_LEFT_POINT_X <= WALL->x2 && BALL_RIGHT_POINT_X > WALL->x2 && BALL_LEFT_POINT_Y >= WALL->y1 && BALL_RIGHT_POINT_Y <= WALL->y2 && ball->dx < 0) {
...
unsigned char checkBallCollisionsWithWalls(ball_t *ball, void (*callback)(void *), void *args)
{
    unsigned char ret = 0;
    unsigned int i;
    for (i = 0; i < walls.wall_count; i++)
        if (handleCollision(ball, walls.walls[i], COLLIDE_WALL, callback, args)) {
            ret = 1;
        }
    return ret;
}
...
unsigned char checkBallCollisions(ball_t *ball, void (*callback)(void *), void *args)
{
    unsigned char ret = 0;
