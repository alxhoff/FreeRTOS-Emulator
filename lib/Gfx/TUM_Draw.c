...
uint32_t SwapBytes(uint x) { return ((x & 0x000000ff) << 24) + ((x & 0x0000ff00) << 8) + ((x & 0x00ff0000) >> 8) + ((x & 0xff000000) >> 24); }
...
char* tumGetErrorMessage(void) { return error_message; }
...
static void vDrawRectangle(signed short x, signed short y, signed short w, signed short h, unsigned int colour) { rectangleColor(renderer, x + w, y, x, y + h, SwapBytes((colour << 8) | 0xFF)); }

static void vDrawFilledRectangle(signed short x, signed short y, signed short w, signed short h, unsigned int colour) { boxColor(renderer, x + w, y, x, y + h, SwapBytes((colour << 8) | 0xFF)); }

static void vDrawArc(signed short x, signed short y, signed short radius, signed short start, signed short end, unsigned int colour) { arcColor(renderer, x, y, radius, start, end, SwapBytes((colour << 8) | 0xFF)); }

static void vDrawEllipse(signed short x, signed short y, signed short rx, signed short ry, unsigned int colour) { ellipseColor(renderer, x, y, rx, ry, SwapBytes((colour << 8) | 0xFF)); }

static void vDrawCircle(signed short x, signed short y, signed short radius, unsigned int colour) { filledCircleColor(renderer, x, y, radius, SwapBytes((colour << 8) | 0xFF)); }

static void vDrawLine(signed short x1, signed short y1, signed short x2, signed short y2, unsigned char thickness, unsigned int colour) { thickLineColor(renderer, x1, y1, x2, y2, thickness, SwapBytes((colour << 8) | 0xFF)); }
...
static void vDrawTriangle(coord_t* points, unsigned int colour) { filledTrigonColor(renderer, points[0].x, points[0].y, points[1].x, points[1].y, points[2].x, points[2].y, SwapBytes((colour << 8) | 0xFF)); }
...
static void vRenderScaledImage(SDL_Texture* tex, SDL_Renderer* ren, unsigned short x, unsigned short y, unsigned short w, unsigned short h)
{
    SDL_Rect dst;
    dst.w = w;
    dst.h = h;
...
static void vDrawScaledImage(SDL_Texture* tex, SDL_Renderer* ren, unsigned short x, unsigned short y, float scale)
{
    int w, h;
    SDL_QueryTexture(tex, NULL, NULL, &w, &h);
...
static void vDrawImage(SDL_Texture* tex, SDL_Renderer* ren, unsigned short x, unsigned short y) { vDrawScaledImage(tex, ren, x, y, 1); }

static void vDrawLoadAndDrawImage(char* filename, SDL_Renderer* ren, unsigned short x, unsigned short y)
{
...
static void vDrawRectImage(SDL_Texture* tex, SDL_Renderer* ren, SDL_Rect dst, SDL_Rect* clip) { SDL_RenderCopy(ren, tex, clip, &dst); }

static void vDrawClippedImage(SDL_Texture* tex, SDL_Renderer* ren, unsigned short x, unsigned short y, SDL_Rect* clip)
{
    SDL_Rect dst;
    dst.x = x;
    dst.y = y;
...
static void vDrawText(char* string, unsigned short x, unsigned short y, unsigned int colour)
{
    SDL_Color color = { RED_PORTION(colour), GREEN_PORTION(colour), BLUE_PORTION(colour), ZERO_ALPHA };
...
static void vDrawArrow(unsigned short x1, unsigned short y1, unsigned short x2, unsigned short y2, unsigned short head_length, unsigned char thickness, unsigned int colour)
{
    // Line vector
    ...
    thickLineColor(renderer, x1, y1, x2, y2, thickness, SwapBytes((colour << 8) | 0xFF));
    thickLineColor(renderer, head_x1, head_y1, x2, y2, thickness, SwapBytes((colour << 8) | 0xFF));
    thickLineColor(renderer, head_x2, head_y2, x2, y2, thickness, SwapBytes((colour << 8) | 0xFF));
}
...
static void vHandleDrawJob(draw_job_t* job)
{
    ...
    case DRAW_CLEAR:
        ...
        break;
    case DRAW_ARC:
        vDrawArc(job->data->arc.x, job->data->arc.y, job->data->arc.radius, job->data->arc.start, job->data->arc.end, job->data->arc.colour);
        break;
    case DRAW_ELLIPSE:
        vDrawEllipse(job->data->ellipse.x, job->data->ellipse.y, job->data->ellipse.rx, job->data->ellipse.ry, job->data->ellipse.colour);
        break;
    case DRAW_TEXT:
        vDrawText(job->data->text.str, job->data->text.x, job->data->text.y, job->data->text.colour);
        free(job->data->text.str);
        break;
    case DRAW_RECT:
        vDrawRectangle(job->data->rect.x, job->data->rect.y, job->data->rect.w, job->data->rect.h, job->data->rect.colour);
        break;
    case DRAW_FILLED_RECT:
        vDrawFilledRectangle(job->data->rect.x, job->data->rect.y, job->data->rect.w, job->data->rect.h, job->data->rect.colour);
        break;
    case DRAW_CIRCLE:
        vDrawCircle(job->data->circle.x, job->data->circle.y, job->data->circle.radius, job->data->circle.colour);
        break;
    case DRAW_LINE:
        vDrawLine(job->data->line.x1, job->data->line.y1, job->data->line.x2, job->data->line.y2, job->data->line.thickness, job->data->line.colour);
        break;
    case DRAW_POLY:
    ...
    case DRAW_IMAGE:
        ...
        vDrawImage(job->data->image.tex, renderer, job->data->image.x, job->data->image.y);
        break;
    case DRAW_SCALED_IMAGE:
        job->data->scaled_image.image.tex = loadImage(job->data->scaled_image.image.filename, renderer);
        vDrawScaledImage(job->data->scaled_image.image.tex, renderer, job->data->scaled_image.image.x, job->data->scaled_image.image.y, job->data->scaled_image.scale);
        break;
    case DRAW_ARROW:
        vDrawArrow(job->data->arrow.x1, job->data->arrow.y1, job->data->arrow.x2, job->data->arrow.y2, job->data->arrow.head_length, job->data->arrow.thickness, job->data->arrow.colour);
    default:
        break;
    }
    free(job->data);
...
signed char tumDrawText(char* str, signed short x, signed short y, unsigned int colour)
{
...
void tumGetTextSize(char* str, unsigned int* width, unsigned int* height) { vGetTextSize(str, width, height); }

signed char tumDrawEllipse(signed short x, signed short y, signed short rx, signed short ry, unsigned int colour)
{
...
signed char tumDrawArc(signed short x, signed short y, signed short radius, signed short start, signed short end, unsigned int colour)
{
...
signed char tumDrawFilledBox(signed short x, signed short y, signed short w, signed short h, unsigned int colour)
{
...
signed char tumDrawBox(signed short x, signed short y, signed short w, signed short h, unsigned int colour)
{
...
signed char tumDrawCircle(signed short x, signed short y, signed short radius, unsigned int colour)
{
...
signed char tumDrawLine(signed short x1, signed short y1, signed short x2, signed short y2, unsigned char thickness, unsigned int colour)
{
...
signed char tumDrawScaledImage(char* filename, signed short x, signed short y, float scale)
{
    // TODO
...
signed char tumDrawArrow(unsigned short x1, unsigned short y1, unsigned short x2, unsigned short y2, unsigned short head_length, unsigned char thickness, unsigned int colour)
{
